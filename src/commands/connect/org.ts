...
const messages = Messages.loadMessages('dependency-updater', 'connect.org');

export type ConnectOrgResult = PackageUpdates[] | null;
...
export default class ConnectOrg extends SfCommand<ConnectOrgResult> {
  ...
  public async run(): Promise<ConnectOrgResult> {
    ...
    const authOrg = flags.username ?? (await ConnectOrg.getDevHub())?.username;
    this.log(authOrg);
    ...
    for (const currentPackage of currentPackages) {
      ...
      if (ConnectOrg.checkIfNewerPackageVersion(currentPackage.versionNumber as string, testPackage?.packageVersion))
    ...
    for (const ret of results) {
      ...
      packs.find(
        (pack: { package: string; versionNumber: string }) => pack.package === ret.packageAlias
      ).versionNumber = replacementText;
    }
